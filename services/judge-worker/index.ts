...
async function runJudge0(languageId: number, source: string, stdin: string, cpu: number, mem: number): Promise<Judge0Result> {
  if (!JUDGE0_URL) {
    return { status: { id: 13, description: "Judge Unavailable" }, compile_output: "CODE_JUDGE not configured" };
  }
  ...
  if (!res.ok) {
    return { status: { id: 13, description: "Internal Error" }, compile_output: "judge_http_error" };
  }
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    const sub = await prisma.codingSubmission.findUnique({
      ...
      include: {
        activity: { include: { codingProblem: { include: { tests: { orderBy: { sortOrder: "asc" } } } } } },
    ...
    for (const test of problem.tests) {
      ...
      await prisma.testCaseResult.upsert({
        ...
        create: {
          ...
          passed,
          points: award,
          stdout: test.hidden ? "" : stdout.slice(0, 4000),
          stderr: test.hidden ? "" : (result.stderr ?? "").slice(0, 2000),
          timeMs: Math.round(Number(result.time ?? 0) * 1000),
        },
        update: {
          passed,
          points: award,
    ...
    await prisma.codingSubmission.update({
      where: { id: sub.id },
      data: { status, pointsAwarded: points, compileOutput: compile.slice(0, 4000), judgedAt: new Date() },
    });

    if (points > 0) {
      const member = await prisma.teamMember.findFirst({
        where: { userId: sub.userId, team: { roomId: sub.activity.roomId } },
      });
      if (member) {
        const best = await prisma.codingSubmission.aggregate({
          where: {
        ...
        if (delta > 0) {
          await prisma.scoreTransaction.create({
            data: {
              roomId: sub.activity.roomId,
              teamId: member.teamId,
              delta,
              reason: "CODING_RESULT",
              refType: "coding_submission",
              refId: sub.id,
              actorUserId: sub.userId,
              idempotencyKey: `score:code:${sub.id}`,
            },
          }).catch(() => undefined);
    ...
    await redisPub.publish(
      `room:${sub.activity.roomId}`,
      JSON.stringify({
        event: "SUBMISSION_RESULT",
        roomId: sub.activity.roomId,
        at: new Date().toISOString(),
        data: { submissionId: sub.id, userId: sub.userId, status, pointsAwarded: points },
      }),
    );
    await redisPub.quit();
  },
...
worker.on("failed", (job, err) => {
  console.error(JSON.stringify({ ts: new Date().toISOString(), msg: "judge_job_failed", jobId: job?.id, err: err.message }));
});
